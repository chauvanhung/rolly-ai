...
def run_seed(db: Session) -> None:
    ...
    print("Starting idempotent database seeding of Buddhist contents...")

    def get_or_create_by_slug(model, data: dict):
        obj = db.query(model).filter(model.slug == data["slug"]).first()
        if obj:
            return obj, False
        obj = model(**data)
        db.add(obj)
    ...
    categories = {}
    for c in cats_data:
        cat, _created = get_or_create_by_slug(Category, c)
    ...
    for t_name in tags_data:
        slug = make_slug(t_name)
        if not db.query(Tag).filter(Tag.slug == slug).first():
            db.add(Tag(name=t_name, slug=slug))
    db.flush()
    ...
    for t in teachers_data:
        teacher, _created = get_or_create_by_slug(Teacher, t)
    ...
    for s in sutras_data:
        ch_list = s.get("chapters", [])
        data = {k: v for k, v in s.items() if k != "chapters"}
        if exists_by_slug(Sutra, data["slug"]):
            continue
        sutra = Sutra(**data)
        db.add(sutra)
        db.flush()
    ...
    for t in talks_data:
        ...
        if exists_by_slug(DharmaTalk, slug):
            continue
        talk = DharmaTalk(
            slug=slug,
            status="published",
    ...
    for l in lectures_data:
        ...
        if exists_by_slug(Lecture, slug):
            continue
        lec = Lecture(
            slug=slug,
            status="published",
    ...
    for r in retreats_data:
        ...
        if exists_by_slug(Retreat, slug):
            continue
        rt = Retreat(
            slug=slug,
            status="published",
    ...
    for c in charity_data:
        ...
        if exists_by_slug(CharityProgram, slug):
            continue
        cp = CharityProgram(
            slug=slug,
            status="published",
    ...
    for n in news_data:
        ...
        if exists_by_slug(NewsPost, slug):
            continue
        np = NewsPost(
            slug=slug,
            status="published",
    ...
    for m in media_data:
        exists = db.query(MediaAsset).filter(
            MediaAsset.file_name == m["file_name"],
            MediaAsset.folder == m.get("folder", "/")
        ).first()
        if not exists:
            db.add(MediaAsset(**m))
    db.flush()
    ...
    for s in sub_data:
        if not db.query(Subscriber).filter(Subscriber.email == s["email"].lower()).first():
            sub = Subscriber(**s)
            db.add(sub)
    ...
    contact_data = [
        ...
        {"full_name": "Lê Thị Dung", "email": "dung@outlook.com", "phone": "0912345678", "subject": "Đăng ký làm công quả khóa tu", "body": "A Di Đà Phật, con muốn xin phép đăng ký làm công quả hỗ trợ chuẩn bị thức ăn cho Khóa tu Mùa hè sắp tới, kính mong quý thầy duyệt giúp con."}
    ]
    for c in contact_data:
        exists = db.query(ContactMessage).filter(
            ContactMessage.email == c["email"],
            ContactMessage.subject == c["subject"]
        ).first()
        if not exists:
    ...
    db.commit()
    print("Database seeding of Buddhist contents completed successfully!")
